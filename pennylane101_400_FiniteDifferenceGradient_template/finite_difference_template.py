...
def my_finite_diff_grad(params):
    ...
    for i in range(len(params)):
        ...
        params_plus_delta = np.array(params, copy=True)
        params_minus_delta = np.array(params, copy=True) 

        params_plus_delta[i] += delta/2
        params_minus_delta[i] -= delta/2

        gradients[i] = (cost(params_plus_delta)-cost(params_minus_delta))/delta
        # QHACK #
