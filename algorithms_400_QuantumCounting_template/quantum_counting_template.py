...
def circuit(indices):
    ...
    QuantumPhaseEstimation(
        diffusion_matrix() @ oracle_matrix(indices),
...
def number_of_solutions(indices):
    ...
    # for p, s in zip (circuit(indices), basis_states(4)):
    #     print("State: {}; Prob: {}".format(s, p))

    theta = np.argmax(circuit(indices)) * 3.14159265359/8
    M = 16*np.sin(theta/2)*np.sin(theta/2)
