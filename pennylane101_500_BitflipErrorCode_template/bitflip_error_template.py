...
def error_wire(circuit_output):
    ...
    # for counter, prob in enumerate(circuit_output):
    #     print("state №{} — |{}>, prob {}".format(counter, basis_states[counter], prob))
    #### DEBUG, PRINTS INFO #### 

    return [
            circuit_output[0]+circuit_output[7], # states 000 and 111
            circuit_output[3]+circuit_output[4], # states 011 and 100
            circuit_output[2]+circuit_output[5], # states 010 and 101
            circuit_output[1]+circuit_output[6]  # states 001 and 110
            ]
    # QHACK #
...
def circuit(p, alpha, tampered_wire):
    ...
    # QHACK #
    qml.CNOT(wires=[0,1])
    qml.CNOT(wires=[1,2]) ### CNOT gates create superposition of |000> + |111>
    ...
    # return something!
    return qml.probs(wires=[0, 1, 2])  #  The returned array is in lexicographic order
