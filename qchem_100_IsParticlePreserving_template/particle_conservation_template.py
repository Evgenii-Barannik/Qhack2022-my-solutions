...
def binary_list(m, n):
    ...
    binary_string = format(m, "0{}b".format(n))
    arr = [int(num) for num in binary_string]
...
def is_particle_preserving(circuit, n):
    ...
    hamming_weights_for_all_states = [np.sum(states) for states in basis_states(n)]
    # print("hamming_weights_for_all_states:\n{}\n".format(hamming_weights_for_all_states))

    for initial_state in basis_states(n):
        resulting_states = np.array([c.item() for c in circuit(initial_state)])
        nonzero_state_indices = np.where(resulting_states != 0j)[0]
        hamming_weights = [hamming_weights_for_all_states[i] for i in nonzero_state_indices]

        # print("resulting_states:\n{}\n".format(resulting_states))
        # print("initial_state:\n{}\n".format(initial_state))
        # print("nonzero_state_indices:\n{}\n".format(nonzero_state_indices))
        # print("hamming_weights:\n{}\n".format(hamming_weights))
        # print("-----------------------------------------------")
        
        if not np.all(hamming_weights == np.sum(initial_state)):
            return False
